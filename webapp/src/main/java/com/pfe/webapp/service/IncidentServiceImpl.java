...
public class IncidentServiceImpl implements IncidentService {
    ...
    /**
     * Get all compte IDs that should receive notifications for an incident
     ...
     */
    private List<Long> getRecipientCompteIdsForIncident(Incident incident) {
        ...
        // 2. Get mission-specific users if incident has a mission (through employe's affectation)
        if (incident.getEmploye() != null) {
            // Find the employee's current mission
            LocalDate currentDate = LocalDate.now();
            List<AffectationEmploye> affectations = affectationEmployeRepository
                    .findByEmployeIdAndDateFinAfterOrDateFinNull(incident.getEmploye().getId(), currentDate);
    ...
    private String getIncidentLink(Long incidentId) {
        return "/pages/safety/incidents/" ;
    }

    private void sendIncidentCreatedNotification(Incident incident) {
        String title = "New Incident Reported";
        String message = String.format("Incident of type '%s' with severity '%s' has been reported",
                incident.getType().getLabel(),
                incident.getNiveauGravite().getLabel());

        if (incident.getEmploye() != null) {
            message += " for employee " + incident.getEmploye().getPrenom() + " " + incident.getEmploye().getNom();
        }

        String link = getIncidentLink(incident.getId());

        List<Long> recipientIds = getRecipientCompteIdsForIncident(incident);
    ...
    private void sendIncidentUpdatedNotification(Incident incident, String oldDescription) {
        String title = "Incident Updated";
        String message = String.format("Incident '%s' (was '%s') has been updated",
    ...
    private void sendIncidentDeletedNotification(String description, Employe employe) {
        String title = "Incident Deleted";
        String message = String.format("Incident '%s' has been deleted", description);

        if (employe != null) {
            message += " for employee " + employe.getPrenom() + " " + employe.getNom();
        }
        ...
        // Get mission-specific users if employee has a mission
        if (employe != null) {
            LocalDate currentDate = LocalDate.now();
            List<AffectationEmploye> affectations = affectationEmployeRepository
            ...
            if (!affectations.isEmpty()) {
                ...
                List<Long> missionSpecificIds = affectationEmployeRepository
                        ...
                        .stream()
                        .map(AffectationEmploye::getEmploye)
                        .filter(emp -> emp.getCompte() != null)
                        .map(emp -> emp.getCompte().getId())
                        .collect(Collectors.toList());
        ...
        if (!recipientIds.isEmpty()) {
            notificationService.createNotificationForUsers(
                    ...
                    title,
                    message,
                    NotificationType.INCIDENT_DELETED,
                    null
            );
        }
    }
    ...
    public IncidentResponseDTO createIncident(IncidentRequestDTO requestDTO) {
        ...
        // ✅ Send notification
        sendIncidentCreatedNotification(savedIncident);
    ...
    public IncidentResponseDTO updateIncident(Long id, IncidentRequestDTO requestDTO) {
        ...
        String oldDescription = existingIncident.getDescription();
        ...
        // ✅ Send notification
        sendIncidentUpdatedNotification(updatedIncident, oldDescription);
    ...
    public void deleteIncident(Long id) {
        ...
        Employe employe = incident.getEmploye();
        ...
        // ✅ Send notification
        sendIncidentDeletedNotification(description, employe);
    }
