...
public interface ReparationRepository extends JpaRepository<Reparation, Long> {
    ...
    // Add to ReparationRepository
    @Query("SELECT r FROM Reparation r WHERE r.mission.id = :missionId")
    List<Reparation> findByMissionId(@Param("missionId") Long missionId);

    @Query("SELECT r FROM Reparation r WHERE r.mission.id = :missionId AND r.dateReparation BETWEEN :startDate AND :endDate")
    List<Reparation> findByMissionIdAndDateReparationBetween(
