...
public interface RessourceRepository extends JpaRepository<Ressource, Long> {
    ...
    // ========== CRITICAL STOCK ==========
    @Query("SELECT new com.pfe.webapp.dto.stats.RessourceCostStatsDTO(r.titre, r.quantite, r.cout * r.quantite) " +
            "FROM Ressource r " +
            "WHERE r.quantite < :threshold " +
            "ORDER BY r.quantite ASC")
    List<RessourceCostStatsDTO> getCriticalStock(@Param("threshold") Double threshold);

    // ========== GET ALL RESSOURCES WITH STOCK INFO ==========
    @Query("SELECT r FROM Ressource r")
    List<Ressource> findAllWithStock();
}