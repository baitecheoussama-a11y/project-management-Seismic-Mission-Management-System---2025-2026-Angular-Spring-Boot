...
public class MissionController {
    ...
    // In MissionController.java - Add this at the top with other autowired dependencies
    ...
    // In MissionController.java - Update the getMissionOverview method
    ...
    public ResponseEntity<Map<String, Object>> getMissionOverview(@PathVariable Long id) {
        ...
        if (mission.getProjects() != null && !mission.getProjects().isEmpty()) {
            for (Project project : mission.getProjects()) {
                ...
                // Get project status using the new date-based calculation
                String statusCode = project.calculateStatus();
        ...
        if (activeProject != null) {
            ...
            // ✅ FIX: Calculate the true progress from activities
            int projectProgress = calculateProjectProgressFromActivities(project.getId());
            ...
            // ✅ Use the calculated progress from activities
            currentProject.put("progress", projectProgress);

            // Get status using date-based calculation
            String statusCode = project.calculateStatus();
            ...
            if (project.getDateStartReelle() != null) {
                currentProject.put("dateStartReelle", project.getDateStartReelle().toString());
            }
            if (project.getDateFinReelle() != null) {
                currentProject.put("dateFinReelle", project.getDateFinReelle().toString());
            }

            // ✅ Add activities count for the progress display
            List<AffectationEquipe> assignments = affectationEquipeRepository.findByProjectId(project.getId());
            int totalActivities = assignments.size();
            int completedActivities = 0;

            for (AffectationEquipe assignment : assignments) {
                if (assignment.getDateFinReelle() != null ||
                        (assignment.getActive() != null && isActivityCompleted(assignment.getActive()))) {
                    completedActivities++;
                }
            }

            currentProject.put("totalActivities", totalActivities);
            currentProject.put("completedActivities", completedActivities);
            ...
            currentProject.put("totalActivities", 0);
            currentProject.put("completedActivities", 0);
        }
    ...
    // ✅ Add helper method to check if an activity is completed
    private boolean isActivityCompleted(Active active) {
        if (active == null) return false;
        // Check if activity has any completed assignments
        if (active.getAffectationEquipes() != null) {
            for (AffectationEquipe assignment : active.getAffectationEquipes()) {
                if (assignment.getDateFinReelle() != null) {
                    return true;
                }
            }
    ...
    // ✅ Add helper method to calculate project progress from activities
    private int calculateProjectProgressFromActivities(Long projectId) {
        List<AffectationEquipe> assignments = affectationEquipeRepository.findByProjectId(projectId);
        ...
        for (AffectationEquipe assignment : assignments) {
            // Activity is completed if dateFinReelle is set
            if (assignment.getDateFinReelle() != null) {
                completedActivities++;
            }
        }
        ...
        // If all activities are completed -> 100%
        // If no activities completed -> 0%
        // Otherwise calculate based on completed/total ratio
        int progressFromActivities = (int) Math.round(((double) completedActivities / totalActivities) * 100);

        // Cap at 100%
        return Math.min(progressFromActivities, 100);
    }
